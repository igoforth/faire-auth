---
title: Installation
description: Learn how to configure Faire Auth in your project.
---
...
Let's start by adding Faire Auth to your project:
...
<Callout type="info">
If you're using a separate client and server setup, make sure to install Faire Auth in both parts of your project.
</Callout>
</Step>
...
```txt title=".env"
FAIRE_AUTH_URL=http://localhost:3000 # Base URL of your app
```
...
And in this file, import Faire Auth and create your auth instance. Make sure to export the auth instance with the variable name `auth` or as a `default` export.
...
Faire Auth requires a database to store user data.
You can easily configure Faire Auth to use SQLite, PostgreSQL, or MySQL, and more!
...
<Tabs items={["drizzle", "prisma", "mongodb"]}>
    ...
    ```ts title="auth.ts"
    import { faireAuth } from "faire-auth";
    import { mongodbAdapter } from "faire-auth/adapters/mongodb";
...
<Step>
    ...
    Faire Auth includes a CLI tool to help manage the schema required by the library.
...
Configure the authentication methods you want to use. Faire Auth comes with built-in support for email/password, and social sign-on providers.
...
<Callout>
  Faire Auth supports any backend framework with standard Request and Response
...
<Tabs items={["next-js", "nuxt", "svelte-kit", "remix", "solid-start", "hono", "cloudflare-workers", "express", "elysia", "tanstack-start", "expo"]} defaultValue="next-js">
    ...
    import { auth } from "$lib/auth"; // path to your auth file
    import { svelteKitHandler } from "faire-auth/svelte-kit";
    ...
    import { auth } from "~/lib/auth"; // path to your auth file
    import { toSolidStartHandler } from "faire-auth/solid-start";
    ...
    <Callout type="info">
    **Node.js AsyncLocalStorage Support**: Faire Auth uses AsyncLocalStorage for async context tracking. To enable this in Cloudflare Workers, add the `nodejs_compat` flag to your `wrangler.toml`:
    ...
    app.listen(port, () => {
        console.log(`Faire Auth app listening on port ${port}`);
    });
    ```
    ...
    const faireAuthView = (context: Context) => {
        const FAIRE_AUTH_ACCEPT_METHODS = ["POST", "GET"]
        // validate request method
        if(FAIRE_AUTH_ACCEPT_METHODS.includes(context.request.method)) {
    ...
    const app = new Elysia().all("/api/auth/*", faireAuthView).listen(3000);
...
The client-side library helps you interact with the auth server. Faire Auth comes with a client for all the popular web frameworks, including vanilla JavaScript.

1. Import `createAuthClient` from the package for your framework (e.g., "faire-auth/react" for React).
...
<Tabs items={["react", "vue", "svelte", "solid",
  "vanilla"]} defaultValue="react">
    ...
    <Tab value="vue" title="lib/auth-client.ts">
            ```ts  title="lib/auth-client.ts"
            import { createAuthClient } from "faire-auth/vue"
            export const authClient = createAuthClient({
                /** The base URL of the server (optional if you're using the same domain) */ // [!code highlight]
                baseURL: "http://localhost:3000" // [!code highlight]
            })
    ...
    <Tab value="svelte" title="lib/auth-client.ts">
            ```ts  title="lib/auth-client.ts"
            import { createAuthClient } from "faire-auth/svelte"
            export const authClient = createAuthClient({
                /** The base URL of the server (optional if you're using the same domain) */ // [!code highlight]
                baseURL: "http://localhost:3000" // [!code highlight]
            })
...
<Step>
### 🎉 That's it!
That's it! You're now ready to use faire-auth in your application. Continue to [basic usage](/docs/basic-usage) to learn how to use the auth instance to sign in users.
</Step>
</Steps>
