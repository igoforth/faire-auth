...
You can test the device authorization flow right now using the Faire Auth CLI:

```bash
npx @faire-auth/cli login
```

This will demonstrate the complete device authorization flow by:
1. Requesting a device code from the Faire Auth demo server
...
<Callout type="info">
  The CLI login command is a demo feature that connects to the Faire Auth demo server to showcase the device authorization flow in action.
</Callout>
...
<Steps>
    <Step>
        ...
        import { deviceAuthorization } from "faire-auth/plugins"; // [!code highlight]
    ...
    <Step>
        ...
        import { deviceAuthorizationClient } from "faire-auth/client/plugins"; // [!code highlight]
...
import { createAuthClient } from "faire-auth/client";
import { deviceAuthorizationClient } from "faire-auth/client/plugins";
...
async function authenticateCLI() {
  console.log("🔐 Faire Auth Device Authorization Demo");
