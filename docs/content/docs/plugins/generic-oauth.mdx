...
<Steps>
  <Step>
    ...
    import { genericOAuth } from "faire-auth/plugins" // [!code highlight]
  ...
  <Step>
    ...
    import { genericOAuthClient } from "faire-auth/client/plugins"
...
The plugin includes built-in error handling for common OAuth issues. Errors are typically redirected to your application's error page with an appropriate error message in the URL parameters. If the callback URL is not provided, the user will be redirected to Faire Auth's default error page.
