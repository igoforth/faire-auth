---
title: API Key
description: API Key plugin for Faire Auth.
---
...
<Steps>
    ...
    <Step>
        ...
        import { apiKeyClient } from "faire-auth/client/plugins"
...
Any time an endpoint in Faire Auth is called that has a valid API key in the headers, you can automatically create a mock session to represent the user by enabling `sessionForAPIKeys` option.
...
For every other endpoint/method, you should utilize Faire Auth's [built-in rate-limiting](/docs/concepts/rate-limit).
