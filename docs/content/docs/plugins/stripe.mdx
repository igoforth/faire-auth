---
title: Stripe
description: Stripe plugin for Faire Auth to manage subscriptions and payments.
---

The Stripe plugin integrates Stripe's payment and subscription functionality with Faire Auth. Since payment and authentication are often tightly coupled, this plugin simplifies the integration of Stripe into your application, handling customer creation, subscription management, and webhook processing.
...
<Steps>
    ...
    <Step>
        ...
        import { stripe } from "@faire-auth/stripe"
    ...
    <Step>
        ...
        import { stripeClient } from "@faire-auth/stripe/client"
...
stripe({
    // ... other options
    createCustomerOnSignUp: true,
    onCustomerCreate: async ({ customer, stripeCustomer, user }, ctx) => {
        // Do something with the newly created customer
        console.log(`Customer ${customer.id} created for user ${user.id}`);
    },
    getCustomerCreateParams: async ({ user, session }, ctx) => {
...
```ts title="auth.ts"
{
    ...
    freeTrial: {
        ...
        onTrialEnd: async ({ subscription, user }, ctx) => {
