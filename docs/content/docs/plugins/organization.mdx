...
<Step>
    ...
    import { organizationClient } from "faire-auth/client/plugins"
...
You can customize organization operations using hooks that run before and after various organization-related activities. Faire Auth provides two ways to configure hooks:
...
For member invitation to work we first need to provide `sendInvitationEmail` to the `faire-auth` instance. This function is responsible for sending the invitation email to the user.
...
You can configure Faire Auth to execute a callback function when an invitation is accepted. This is useful for logging events, updating analytics, sending notifications, or any other custom logic you need to run when someone joins your organization.
...
```ts title="auth-client.ts"
const invitations = await authClient.organization.listUserInvitations.$get();
```
...
<Steps>
    ...
    import { createAccessControl } from "faire-auth/plugins/access";
    ...
    ```ts title="permissions.ts"
    import { createAccessControl } from "faire-auth/plugins/access";
    import { defaultStatements, adminAc } from 'faire-auth/plugins/organization/access'
...
```ts title="auth-client.ts"
const canCreateProject = await authClient.organization.hasPermission.$post({
  json: {
    permissions: {
      project: ["create"],
    },
...
const canCreateProjectAndCreateSale =
  await authClient.organization.hasPermission.$post({
    json: {
      permissions: {
        project: ["create"],
        sale: ["create"],
...
```ts title="auth-client.ts"
import { createAuthClient } from "faire-auth/client"
import { organizationClient, inferOrgAdditionalFields } from "faire-auth/client/plugins"
...
- `maximumTeams`: Limit the number of teams per organization
  ...
  teams: {
    ...
    // OR
    maximumMembersPerTeam: async ({ teamId, session, organizationId }) => {
...
```ts
await authClient.organization.inviteMember.$post({
  json: {
    email: "user@example.com",
    role: "member",
    teamId: "team-id",
...
Starting with [Faire Auth v1.3](https://github.com/igoforth/faire-auth/releases/tag/v1.3.0), you can easily add custom fields to the `organization`, `invitation`, `member`, and `team` tables.
