---
title: Polar
description: Faire Auth Plugin for Payment and Checkouts using Polar
---

[Polar](https://polar.sh) is a developer first payment infrastructure. Out of the box it provides a lot of developer first integrations for payments, checkouts and more. This plugin helps you integrate Polar with Faire Auth to make your auth + payments flow seamless.
...
```bash
pnpm add faire-auth @polar-sh/faire-auth @polar-sh/sdk
```
...
import { polar, checkout, portal, usage, webhooks } from "@polar-sh/faire-auth";
...
### Configuring FaireAuth Client

You will be using the FaireAuth Client to interact with the Polar functionalities.

```typescript
import { createAuthClient } from "faire-auth/react";
import { polarClient } from "@polar-sh/faire-auth";

// This is all that is needed
// All Polar plugins, etc. should be attached to the server-side FaireAuth config
...
When `createCustomerOnSignUp` is enabled, a new Polar Customer is automatically created when a new User is added in the Faire Auth Database.
...
```typescript
import { polar, checkout } from "@polar-sh/faire-auth";
...
When checkouts are enabled, you're able to initialize Checkout Sessions using the checkout-method on the FaireAuth Client. This will redirect the user to the Product Checkout.
...
```typescript
import { polar, checkout, portal } from "@polar-sh/faire-auth";
...
The portal-plugin gives the FaireAuth Client a set of customer management methods, scoped under `authClient.customer`.
...
```typescript
import { polar, checkout, portal, usage } from "@polar-sh/faire-auth";
...
```typescript
import { polar, webhooks } from "@polar-sh/faire-auth";
