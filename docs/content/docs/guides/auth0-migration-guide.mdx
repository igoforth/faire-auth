---
title: Migrating from Auth0 to Faire Auth
description: A step-by-step guide to transitioning from Auth0 to Faire Auth.
---

In this guide, we'll walk through the steps to migrate a project from Auth0 to Faire Auth — including email/password with proper hashing, social/external accounts, two-factor authentication, and more.
...
Before starting the migration process, set up Faire Auth in your project. Follow the [installation guide](/docs/installation) to get started.
...
import { Pool } from "pg";
import { faireAuth } from "faire-auth";
import { admin, twoFactor, username } from "faire-auth/plugins";
...
import { generateRandomString, symmetricEncrypt } from "faire-auth/crypto";
...
function mapAuth0RoleToFaireAuthRole(auth0Roles: string[]) {
            ...
            try {
                ...
                for (const member of members.data || []) {
                    try {
                        ...
                        const role = mapAuth0RoleToFaireAuthRole(userRoles.data?.map(r => r.name) || []);
                        await ctx.adapter.create({
                            model: "member",
                            data: {
            ...
            try {
                ...
                const baseUserData = {
                    ...
                    ...(isAdminEnabled ? {
                        ...
                        role: mapAuth0RoleToFaireAuthRole(auth0User.roles || []),
                    } : {}),
...
3. Verify the migrated data in Faire Auth before proceeding
...
<Step>
 ...
 By default, Faire Auth uses the `scrypt` algorithm to hash passwords. Since Auth0 uses `bcrypt`, you'll need to configure Faire Auth to use bcrypt for password verification.
...
Now that the data is migrated, update your components to use Faire Auth. Here's an example for the sign-in component:

```tsx title="components/auth/sign-in.tsx"
import { authClient } from "faire-auth/client";
...
Replace your Auth0 middleware with Faire Auth's middleware:

```ts title="middleware.ts"
import { NextRequest, NextResponse } from "next/server";
import { getSessionCookie } from "faire-auth/cookies";
...
Once you've verified everything is working correctly with Faire Auth, remove Auth0:
...
### Role Mapping
The script includes a basic role mapping function (`mapAuth0RoleToFaireAuthRole`). Customize this function based on your Auth0 roles and Faire Auth role requirements.
...
Now! You've successfully migrated from Auth0 to Faire Auth.

Faire Auth offers greater flexibility and more features—be sure to explore the [documentation](/docs) to unlock its full potential.
