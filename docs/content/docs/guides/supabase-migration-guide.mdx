---
title: Migrating from Supabase Auth to Faire Auth
description: A step-by-step guide to transitioning from Supabase Auth to Faire Auth.
---

In this guide, we'll walk through the steps to migrate a project from Supabase Auth to Faire Auth.
...
```ts title="auth.ts"
import { admin, anonymous } from "faire-auth/plugins";
...
Now that we have the necessary tables in our database, we can run the migration script to migrate the users and accounts from Supabase to Faire Auth.
...
Run the migration script to migrate the users and accounts from Supabase to Faire Auth.
...
<Step>
 ...
 By default, Faire Auth uses the `scrypt` algorithm to hash passwords. Since Supabase uses `bcrypt`, you'll need to configure Faire Auth to use bcrypt for password verification.
...
Update your codebase from Supabase auth calls to Faire Auth API.

Here's a list of the Supabase auth API calls and their Faire Auth counterparts.
...
Congratulations! You've successfully migrated from Supabase Auth to Faire Auth.
