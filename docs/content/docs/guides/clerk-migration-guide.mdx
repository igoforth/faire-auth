---
title: Migrating from Clerk to Faire Auth
description: A step-by-step guide to transitioning from Clerk to Faire Auth.
---

In this guide, we'll walk through the steps to migrate a project from Clerk to Faire Auth — including email/password with proper hashing, social/external accounts, phone number, two-factor data, and more.
...
Before starting the migration process, set up Faire Auth in your project. Follow the [installation guide](/docs/installation) to get started. And go to
...
import { Pool } from "pg";
import { faireAuth } from "faire-auth";
import { admin, twoFactor, phoneNumber, username } from "faire-auth/plugins";
...
<Step>
  ...
  Now that the data is migrated, you can start updating your components to use Faire Auth. Here's an example for the sign-in component:
...
<Step>
  ...
  Replace your Clerk middleware with Faire Auth's middleware:
  ...
  Once you've verified that everything is working correctly with Faire Auth, you can remove Clerk:
...
[Goodbye Clerk, Hello Faire Auth – Full Migration Guide!](https://www.youtube.com/watch?v=Za_QihbDSuk)

## Wrapping Up

Congratulations! You've successfully migrated from Clerk to Faire Auth.
