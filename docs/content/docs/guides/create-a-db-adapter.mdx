...
description: Learn how to create a custom database adapter for Faire Auth
---

Learn how to create a custom database adapter for Faire Auth using `createAdapter`.

Our `createAdapter` function is designed to be very flexible, and we've done our best to make it easy to understand and use.
Our hope is to allow you to focus on writing database logic, and not have to worry about how the adapter is working with Faire Auth.
...
```ts
import { createAdapter, type DBAdapterDebugLogOption } from "faire-auth/adapters";
...
The `config` object is mostly used to provide information about the adapter to Faire Auth.
...
- `options`: The Faire Auth options.
- `schema`: The schema from the user's Faire Auth instance.
...
<Callout>
Note:
If the user has enabled the `useNumberId` option, or if `generateId` is `false` in the user's Faire Auth config,
...
The `createSchema` method allows the [Faire Auth CLI](/docs/concepts/cli) to [generate](/docs/concepts/cli/#generate) a schema for the database.

parameters:

- `tables`: The tables from the user's Faire Auth instance schema; which is expected to be generated into the schema file.
...
import { runAdapterTest } from "faire-auth/adapters/test";
import { myAdapter } from "./my-adapter";

describe("My Adapter Tests", async (test) => {
  afterAll(async () => {
    // Run DB cleanup here...
  });
  ...
  runAdapterTest({
    getAdapter: async (faireAuthOptions = {}) => {
      return adapter(faireAuthOptions);
    },
  });
});
```
...
import { expect, test, describe } from "vitest";
import { runNumberIdAdapterTest } from "faire-auth/adapters/test";
import { myAdapter } from "./my-adapter";

describe("My Adapter Numeric ID Tests", async (test) => {
  afterAll(async () => {
    // Run DB cleanup here...
  });
  const adapter = myAdapter({
...
The `config` object is used to provide information about the adapter to Faire Auth.
...
mapKeysTransformOutput: () => {
  return {
    _id: "id", // We want to replace `_id` (from MongoDB) to `id` (for Faire Auth)
  };
},
```
