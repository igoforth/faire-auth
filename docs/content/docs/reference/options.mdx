---
title: Options
description: Faire Auth configuration options reference.
---

List of all the available options for configuring Faire Auth. See [Faire Auth Options](https://github.com/igoforth/faire-auth/blob/main/packages/faire-auth/src/types/options.ts#L13).
...
Faire Auth supports various database configurations including [PostgreSQL](/docs/adapters/postgresql), [MySQL](/docs/adapters/mysql), and [SQLite](/docs/adapters/sqlite).
...
- `revokeSessionsOnPasswordReset`: Revoke all sessions when password is reset (default: `false`)
...
- `freshAge`: How old a session can be and still be considered "fresh" for sensitive operations like account deletion (default: `86400` - 1 day, set `0` to disable)
- `cookieCache`: Enable caching session in cookie
  - `strategy`: Encoding strategy — `"compact"` (base64url + HMAC), `"jwt"` (JWT spec), or `"jwe"` (encrypted) (default: `"compact"`)
  - `refreshCache`: Auto-refresh the cache before expiry without hitting the DB. Set `true` for default (80% of maxAge) or `{ updateAge: seconds }` for custom (default: `false`)
  - `version`: Bump this string to invalidate all existing cookie caches (default: `"1"`)
...
- `onError`: Custom error handler — receives `(error, ctx)` where `ctx` is the Hono Context
- `errorURL`: URL to redirect to on error (default: `/api/auth/error`)
- `exposeMessage`: Expose unhandled error messages to client (default: `false`, use only for debugging)
...
For more details, see the [Hooks & Middleware documentation](/docs/concepts/hooks#middleware--gate-the-request).
...
For more details, see the [Hooks & Middleware documentation](/docs/concepts/hooks#routehooks--intercept-after-validation).
...
For more details, see the [Hooks & Middleware documentation](/docs/concepts/hooks#dto--shape-the-response).
...
- `init`: Hono constructor options passed to the internal Hono instance
- `openapi.enabled`: Enable the OpenAPI JSON endpoint (default: `false`)
- `openapi.path`: Path for the OpenAPI endpoint, relative to base path (default: `"/openapi.json"`)
- `openapi.version`: OpenAPI spec version — `"3"` or `"3.1"` (default: `"3.1"`)
- `openapi.config`: Full OpenAPI spec configuration object (info, servers, security, tags)
