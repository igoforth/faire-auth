---
title: Telemetry
description: Faire Auth now collects anonymous telemetry data about general usage.
---

Faire Auth collects anonymous usage data to help us improve the project. This is optional, transparent, and disabled by default.

## Why is telemetry collected?

Since v1.3.5, Faire Auth collects anonymous telemetry data about general usage if enabled.

Telemetry data helps us understand how Faire Auth is being used across different environments so we can improve performance, prioritize features, and fix issues more effectively. It guides our decisions on performance optimizations, feature development, and bug fixes. All data is collected completely anonymously and with privacy in mind, and users can opt out at any time. We strive to keep what we collect as transparent as possible.
...
- **Redacted auth config snapshot**: A minimized, privacy‑preserving view of your `faireAuth` options produced by `getTelemetryAuthConfig`.
...
<Callout type="info">
  You can audit telemetry locally by setting the `FAIRE_AUTH_TELEMETRY_DEBUG=1` environment variable when running your project or by setting `telemetry: { debug: true }` in your auth config. In this debug mode, telemetry events are logged only to the console.
...
All collected data is fully anonymous and only useful in aggregate. It cannot be traced back to any individual source and is accessible only to a small group of core Faire Auth maintainers to guide roadmap decisions.

- **No PII or secrets**: We do not collect emails, usernames, tokens, secrets, client IDs, client secrets, or database URLs.
- **No full config**: We never send your full `faireAuth` configuration. Instead we send a reduced, redacted snapshot of non‑sensitive toggles and counts.
- **Redaction by design**: See [detect-auth-config.ts](https://github.com/igoforth/faire-auth/blob/main/packages/faire-auth/src/telemetry/detectors/detect-auth-config.ts) in the Faire Auth source for the exact shape of what is included. It purposely converts sensitive values to booleans, counts, or generic identifiers.
...
- Via an environment variable.
  ...
  ```txt title=".env"
  # Enable telemetry
  FAIRE_AUTH_TELEMETRY=1

  # Disable telemetry
  FAIRE_AUTH_TELEMETRY=0
  ```


### When is telemetry sent?

- On `faireAuth` initialization (`type: "init"`).
