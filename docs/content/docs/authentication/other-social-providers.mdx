...
Faire Auth provides out of the box support for a [Generic OAuth Plugin](/docs/plugins/generic-oauth) which allows you to use any social provider that implements the OAuth2 protocol or OpenID Connect (OIDC) flows.
...
```ts title="auth.ts"
import { faireAuth } from "faire-auth";
import { genericOAuth } from "faire-auth/plugins";
