...
description: Integrate Faire Auth with SvelteKit.
---
...
export async function handle({ event, resolve }) {
  // Fetch current session from Faire Auth
...
You need to add it as a plugin to your Faire Auth instance.
...
import { createAuthClient } from "faire-auth/svelte"; // make sure to import from faire-auth/svelte
