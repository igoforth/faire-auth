...
description: Integrate Faire Auth with Astro.
---

Faire Auth comes with first class support for Astro. This guide will show you how to integrate Faire Auth with Astro.
...
To enable Faire Auth to handle requests, we need to mount the handler to a catch all API route. Create a file inside `/pages/api/auth` called `[...all].ts` and add the following code:
...
declare namespace App {
    ...
    interface Locals {
        user: import("faire-auth").User | null;
        session: import("faire-auth").Session | null;
    }
}
```
