...
description: Integrate Faire Auth with Waku.
---

Faire Auth can be easily integrated with Waku. Before you start, make sure you have a Faire Auth instance configured. If you haven't done that yet, check out the [installation](/docs/installation).

## Create auth instance

Create a file named `auth.ts` in your application. Import Faire Auth and create your instance.
...
<Callout type="info">
 You can change the path on your faire-auth configuration but it's recommended to keep it as `src/pages/api/auth/[...route].ts`
</Callout>
...
import { createAuthClient } from "faire-auth/react" // make sure to import from faire-auth/react
...
The `api` object exported from the auth instance contains all the actions that you can perform on the server. Every endpoint made inside Faire Auth is a invocable as a function. Including plugins endpoints.
...
```ts title="auth.ts"
import { faireAuth } from "faire-auth";
import { wakuCookies } from "faire-auth/waku";
      ...
      after: [
        {
          ...
          handler: createHook()(async (ctx) => {
            ...
            if (returned instanceof Headers) {
              ...
              contextData.faireAuthSetCookie = setCookieHeader;
            }
          }),
        },
      ],
...
See below for the middleware to create to add the `contextData.faireAuthSetCookie` cookies to the response.
...
You can use the `getSessionCookie` helper from Faire Auth for this purpose:
...
const authMiddleware: Middleware = () => {
    return async (ctx, next) => {
        ...
        getSession()
        await next()
        if (ctx.data.faireAuthSetCookie) {
            ctx.res.headers ||= {}
            ...
            ctx.res.headers["set-cookie"] = [
                ...origSetCookie,
                ctx.data.faireAuthSetCookie as string,
            ]
        }
    }
};
...
```ts
import { getCookieCache } from "faire-auth/cookies"
