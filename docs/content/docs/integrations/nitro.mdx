...
description: Integrate Faire Auth with Nitro.
---

Faire Auth can be integrated with your [Nitro Application](https://nitro.build/) (an open source framework to build web servers).

This guide aims to help you integrate Faire Auth with your Nitro application in a few simple steps.
...
### Install & Configure Faire Auth

Follow steps 1 & 2 from the [installation guide](/docs/installation) to install Faire Auth in your Nitro application & set up the environment variables.

Once that is done, create your Faire Auth instance within the `server/utils/auth.ts` file.
...
Use the Faire Auth CLI to update your Prisma schema with the required models by running the following command:

```bash title="Terminal"
npx @faire-auth/cli generate --config server/utils/auth.ts
```

<Callout>
  The `--config` flag is used to specify the path to the file where you have created your Faire Auth instance.
</Callout>
...
You can now mount the Faire Auth handler in your Nitro application. You can do this by adding the following code to your `server/routes/api/auth/[...all].ts` file:
...
You can find an example of a Nitro application integrated with Faire Auth & Prisma [here](https://github.com/BayBreezy/nitrojs-faire-auth-prisma).
