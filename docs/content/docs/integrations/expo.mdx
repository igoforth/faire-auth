...
description: Integrate Faire Auth with Expo.
---

Expo is a popular framework for building cross-platform apps with React Native. Faire Auth supports both Expo native and web apps.

## Installation

<Steps>
    <Step>
        ## Configure A Faire Auth Backend
        Before using Faire Auth with Expo, make sure you have a Faire Auth backend set up. You can either use a separate server or leverage Expo's new [API Routes](https://docs.expo.dev/router/reference/api-routes) feature to host your Faire Auth instance.

        To get started, check out our [installation](/docs/installation) guide for setting up Faire Auth on your server. If you prefer to check out the full example, you can find it [here](https://github.com/faire-auth/examples/tree/main/expo-example).

        To use the new API routes feature in Expo to host your Faire Auth instance you can create a new API route in your Expo app and mount the Faire Auth handler.

        ```ts title="app/api/auth/[...auth]+api.ts"
        import { auth } from "@/lib/auth"; // import Faire Auth handler
    ...
    <Step>
        ...
        Install both the Faire Auth package and Expo plugin into your server application.

        ```package-install
        faire-auth @faire-auth/expo
        ```
    ...
    <Step>
        ...
        You also need to install both the Faire Auth package and Expo plugin into your Expo application.
    ...
    <Step>
        ...
        Add the Expo plugin to your Faire Auth server.
    ...
    <Step>
        ## Initialize Faire Auth Client

        To initialize Faire Auth in your Expo app, you need to call `createAuthClient` with the base URL of your Faire Auth backend. Make sure to import the client from `/react`.
        ...
        You need to also import client plugin from `@faire-auth/expo/client` and pass it to the `plugins` array when initializing the auth client.
        ...
        export const authClient = createAuthClient()({
            baseURL: "http://localhost:8081", // Base URL of your Faire Auth backend.
            plugins: [
                expoClient({
                    scheme: "myapp",
                    storagePrefix: "myapp",
     ...
     <Step>
        ...
        Faire Auth uses deep links to redirect users back to your app after authentication. To enable this, you need to add your app's scheme to the `trustedOrigins` list in your Faire Auth config.
        ...
        Then, update your Faire Auth config to include the scheme in the `trustedOrigins` list.
    ...
    <Step>
        ...
        To resolve Faire Auth exports you'll need to enable `unstable_enablePackageExports` in your metro config.
            ...
            return {
                ...
                plugins: [
                    [
                        ...
                        {
                            alias: {
                                "faire-auth/react": "./node_modules/faire-auth/dist/client/react/index.cjs",
                                "faire-auth/client/plugins": "./node_modules/faire-auth/dist/client/plugins/index.cjs",
                                "@faire-auth/expo/client": "./node_modules/@faire-auth/expo/dist/client.cjs",
                            },
                        },
                    ],
                ],
...
With Faire Auth initialized, you can now use the `authClient` to authenticate users in your Expo app.
...
export default function SocialSignIn() {
    const handleLogin = async () => {
        await authClient.signIn.social.$post({
            json: {
                provider: "google",
                callbackURL: "/dashboard", // converted to a deep link (eg. `myapp://dashboard`) on native
            },
        })
    };
...
Faire Auth provides a `useSession` hook to access the current user's session in your app.
...
**scheme**: scheme is used to deep link back to your app after a user has authenticated using oAuth providers. By default, Faire Auth tries to read the scheme from the `app.json` file. If you need to override this, you can pass the scheme option to the client.
