---
title: Cookies
description: Learn how cookies are used in Faire Auth.
---
...
By default, Faire Auth cookies follow the format `${prefix}.${cookie_name}`. The default prefix is "faire-auth". You can change the prefix by setting `cookiePrefix` in the `advanced` object of the auth options.
...
By default, Faire Auth uses the following cookies:
