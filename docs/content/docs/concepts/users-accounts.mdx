...
Beyond authenticating users, Faire Auth also provides a set of methods to manage users. This includes, updating user information, changing passwords, and more.
...
```ts
await authClient.updateUser.$post({
    json: {
...
export const auth = faireAuth({
    user: {
        changeEmail: {
            enabled: true,
            sendChangeEmailVerification: async ({ user, newEmail, url, token }, ctx) => {
                await sendEmail({
...
```ts
await authClient.changeEmail.$post({
    json: {
...
Faire Auth provides a utility to hard delete a user from your database. It's disabled by default, but you can enable it easily by passing `enabled:true`
...
For added security, you’ll likely want to confirm the user’s intent before deleting their account. A common approach is to send a verification email. Faire Auth provides a `sendDeleteAccountVerification` utility for this purpose.
    ...
    user: {
        deleteUser: {
            ...
            sendDeleteAccountVerification: async (
                ...
                ctx  // The Hono context (access request via ctx.req.raw)
            ) => {
...
await authClient.deleteUser.$post({
    json: {
...
```ts title="delete-user.ts"
await authClient.deleteUser.$post({ json: {} });
```
...
Faire Auth supports multiple authentication methods. Each authentication method is called a provider. For example, email and password authentication is a provider, Google authentication is a provider, etc.
...
Account linking enables users to associate multiple authentication methods with a single account. With Faire Auth, users can connect additional social sign-ons or OAuth providers to their existing accounts if the provider confirms the user's email as verified.
...
- **Linking Social Accounts:** Use the `linkSocial` method on the client to link a social provider to the user's account.
  ...
  ```ts
  await authClient.linkSocial.$post({
      json: {
...
```ts
await authClient.unlinkAccount.$post({
    json: { providerId: "google" },
});

// Unlink a specific account
await authClient.unlinkAccount.$post({
    json: { providerId: "google", accountId: "123" },
});
```
