...
The Faire Auth auth client on the frontend uses the `createAuthClient` function provided by `faire-auth/client`.
...
To create an Auth Endpoint you'll need to import `createAuthEndpoint` from `faire-auth`.

Faire Auth uses wraps around another library called <Link href="https://github.com/bekacru/better-call"> Better Call </Link> to create endpoints. Better call is a simple ts web framework made by the same team behind Faire Auth.

```ts title="plugin.ts"
import { createAuthEndpoint } from "faire-auth/api";
...
Create Auth endpoints wraps around `createEndpoint` from Better Call. Inside the `ctx` object, it'll provide another object called `context` that give you access faire-auth specific contexts including `options`, `db`, `baseURL` and more.

**Context Object**

- `appName`: The name of the application. Defaults to "Faire Auth".
- `options`: The options passed to the Faire Auth instance.
...
- `logger`: The logger instance used by Faire Auth.
- `db`: The Kysely instance used by Faire Auth to interact with the database.
...
- `internalAdapter`: These are internal db calls that are used by Faire Auth. For example, you can use these calls to create a session instead of using `adapter` directly. `internalAdapter.createSession(userId)`
...
For other properties, you can check the <Link href="https://github.com/bekacru/better-call">Better Call</Link> documentation and the <Link href="https://github.com/igoforth/faire-auth/blob/main/packages/faire-auth/src/init.ts">source code </Link>.
...
```ts title="plugin.ts"
import { FaireAuthPlugin } from "faire-auth/plugins";
...
```ts title="plugin.ts"
import {  createAuthMiddleware } from "faire-auth/plugins";
...
```ts title="plugin.ts"
import {  createAuthMiddleware } from "faire-auth/plugins";
import { getSessionFromCtx } from "faire-auth/api";
...
```ts title="plugin.ts"
import { createAuthMiddleware } from "faire-auth/plugins";
import { sessionMiddleware } from "faire-auth/api";
...
If your endpoints needs to be called from the client, you'll need to also create a client plugin. Faire Auth clients can infer the endpoints from the server plugins. You can also add additional client side logic.
...
Faire Auth uses <Link href="https://better-fetch.vercel.app"> Better fetch </Link> to make requests. Better fetch is a simple fetch wrapper made by the same author of Faire Auth.
