...
Faire Auth is designed to be type-safe. Both the client and server are built with TypeScript, allowing you to easily infer types.
...
```ts
import { inferAdditionalFields } from "faire-auth/client/plugins";
