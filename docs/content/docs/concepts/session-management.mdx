...
description: Faire Auth session management.
---

Faire Auth manages session using a traditional cookie-based session management. The session is stored in a cookie and is sent to the server on every request. The server then verifies the session and returns the user data if the session is valid.
...
Some endpoints in Faire Auth require the session to be **fresh**. A session is considered fresh if its `createdAt` is within the `freshAge` limit. By default, the `freshAge` is set to **1 day** (60 * 60 * 24).
...
Faire Auth provides a set of functions to manage sessions.
...
const { data: session } = await authClient.getSession.$get({ query: {} })
```
...
const sessions = await authClient.listSessions.$get()
```
...
```ts title="auth-client.ts"
await authClient.revokeSession.$post({
...
```ts title="auth-client.ts"
await authClient.revokeOtherSessions.$post()
```
...
```ts title="auth-client.ts"
await authClient.revokeSessions.$post()
```
...
```ts title="auth.ts"
await authClient.changePassword.$post({
    json: {
...
```ts title="auth-client.ts"
const session = await authClient.getSession.$get({
...
```ts title="auth.ts"
import { customSession } from "faire-auth/plugins";
...
```ts title="auth-client.ts"
import { customSessionClient } from "faire-auth/client/plugins";
...
const { data: sessionData } = await authClient.getSession.$get({ query: {} });
// data.roles
// data.user.newField
```
...
```ts
import { faireAuth, FaireAuthOptions } from "faire-auth";
...
The `/multi-session/list-device-sessions` endpoint is used to list the devices that the user is signed into.
