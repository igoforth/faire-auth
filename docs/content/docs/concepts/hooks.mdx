---
title: Hooks
description: Faire Auth Hooks let you customize FaireAuth's behavior
---

Hooks in Faire Auth let you "hook into" the lifecycle and execute custom logic. They provide a way to customize Faire Auth's behavior without writing a full plugin.

<Callout>
We highly recommend using hooks if you need to make custom adjustments to an endpoint rather than making another endpoint outside of Faire Auth.
</Callout>
...
import { createAuthMiddleware, APIError } from "faire-auth/api";
...
```ts title="auth.ts"
import { faireAuth } from "faire-auth";
import { createAuthMiddleware } from "faire-auth/api";
...
Access FaireAuth’s predefined cookie properties:
...
Adapter exposes the adapter methods used by Faire Auth. Including `findOne`, `findMany`, `create`, `delete`, `update` and `updateMany`. You generally should use your actually `db` instance from your orm rather than this adapter.
