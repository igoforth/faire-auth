---
title: Client
description: Faire Auth client library for authentication.
---

Faire Auth offers a client library compatible with popular frontend frameworks like React, Vue, Svelte, and more. This client library includes a set of functions for interacting with the Faire Auth server. Each framework's client library is built on top of a core client library that is framework-agnostic, so that all methods and hooks are consistently available across all client libraries.

## Installation

If you haven't already, install faire-auth.
...
Import `createAuthClient` from the package for your framework (e.g., "faire-auth/react" for React). Call the function to create your client. Pass the base URL of your auth server. If the auth server is running on the same domain as your client, you can skip this step.
...
Once you've created your client instance, you can use the client to interact with the Faire Auth server. The client provides a set of functions by default and they can be extended with plugins.
...
<Tabs items={["React", "Vue","Svelte", "Solid"]} defaultValue="react">
    ...
    <Tab value="Vue">
        ...
        <template>
            <div>
                <button v-if="!session.data" @click="() => authClient.signIn.social.$post({
...
Better fetch is a wrapper around the native fetch API that provides a more convenient way to make requests. It's created by the same team behind Faire Auth and is designed to work seamlessly with it.
...
You can also pass fetch options to most of the client functions as the second argument:
...
const { error } = await authClient.signUp.email.$post({
	json: {
...
If you use plugins that add routes (most plugins do), you **must** pass the inferred `App` type to `createAuthClient` so the client knows about the plugin's routes:

```ts title="auth-client.ts"
import { createAuthClient } from "faire-auth/client"
import { magicLinkClient } from "faire-auth/client/plugins"
...
<Callout type="warn">
Without the `<typeof App>` generic, the client won't have type information for plugin routes. You'll be able to call core routes like `signIn.email.$post()`, but plugin-specific routes won't be typed or may not work at all.
</Callout>

Once you've added the plugin, you can use the new functions provided by the plugin.

```ts title="auth-client.ts"
await authClient.signIn.magicLink.$post({
