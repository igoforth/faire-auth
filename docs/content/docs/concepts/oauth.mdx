---
title: OAuth
description: How Faire Auth handles OAuth
---

Faire Auth comes with built-in support for OAuth 2.0 and OpenID Connect. This allows you to authenticate users via popular OAuth providers like Google, Facebook, GitHub, and more.

If your desired provider isn't directly supported, you can implement a custom OAuth provider using the social providers API.
...
await authClient.signIn.social.$post({
  json: { provider: "google" }, // or any other provider id
})
```
...
```ts
const { data } = await authClient.getAccessToken.$post({
  json: {
...
await auth.api.getAccessToken(
  {
    json: {
      providerId: "google",
      accountId: "accountId", // optional
      userId: "userId", // optional, if you don't provide headers
    },
  },
  { headers: /* pass headers with authenticated token */ },
);
...
```ts
const { data: info } = await authClient.accountInfo.$post({
  json: { accountId: "accountId" }, // the provider is automatically detected from the account id
})
```
...
<Callout>
Make sure you're running Faire Auth version 1.2.7 or later. Earlier versions (like 1.2.2) may show a "Social account already linked" error when trying to link with an existing provider for additional scopes.
</Callout>
