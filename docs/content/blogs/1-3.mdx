...
## Faire Auth 1.3 Release

We're excited to announce the release of Faire Auth 1.3. This release includes a lot of new features and improvements.

To upgrade, run:

```package-install
npm install faire-auth@1.3
```
...
import { sso } from "@faire-auth/sso";
...
```ts title="auth.ts"
import { mcp } from "faire-auth/plugins";
...
```ts title="auth.ts"
import { siwe } from "faire-auth/plugins";
...
```ts title="auth.ts"
import { faireAuth } from "faire-auth";
import { sveltekitCookies } from "faire-auth/svelte-kit";
...
👉 [Check the full changelog](https://github.com/igoforth/faire-auth/releases/tag/v1.3.0)
