---
title: Migrate from Supabase Auth to Faire Auth + PlanetScale PostgreSQL
description: This migration guide aims to guide you move your auth from Supabase Auth to Faire Auth on PlanetScale PostgreSQL.
date: 2025-08-25
author:
  name: "Dagmawi Esayas"
...
# Supabase Auth to Faire Auth + PlanetScale PostgreSQL Migration Guide
...
We’ve noticed that some users are migrating from Supabase to PlanetScale PostgreSQL, but facing challenges because they also rely on Supabase Auth. This guide will help you migrate your authentication from Supabase Auth to Faire Auth on PlanetScale PostgreSQL.
...
<Step>
  Create a [new database](<[https://app.planetscale.com/new](https://app.planetscale.com/new?org=faire-auth)>)
</Step>
<Step>
...
<Step>
  Save the database URL in your `.env` file for later use with Faire Auth:
```txt title=".env"
DATABASE_URL =
  postgresql://<username>:<password>@<host>/postgres?sslmode=verify-full
```
...
<Step>
  ...
  [here](https://www.faire-auth.com/docs/installation)
</Step>
</Steps>
...
## 4. Generate & Migrate Faire Auth Schema

<Steps>
<Step>
  Run this cli command to generate all the schema needed to setup Faire Auth:
```bash
...
Now comes the fun part. You are now all setup to move your auth from Supabase Auth to Faire Auth and all you have to do is go through the instances you've used Supabase Auth client and replace it with Faire Auth client. We are going to see a few examples here.
...
<Callout type="info">
  ...
  guide](https://www.faire-auth.com/docs/guides/supabase-migration-guide) we
  made.
</Callout>
...
You've successfully migrated from Supabase Auth to Faire Auth on PlanetScale.
...
- Remember that this is just the basics and if you've integrated Supabase Auth's auth functions in a lot of placed you'd have to find the suitable Faire Auth replacements [here](https://www.faire-auth.com/docs).
- Have fun!
...
<Cards>
  <Card
    href="https://www.faire-auth.com/docs/introduction"
    title="Faire Auth Setup"
  >
    Get started with installing Faire Auth
  </Card>
  <Card
  ...
  <Card
    href="https://www.faire-auth.com/docs/guides/supabase-migration-guide"
