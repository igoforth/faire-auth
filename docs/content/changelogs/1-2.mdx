...
# Faire Auth 1.2 – Stripe, Captcha, API Keys, Teams, Init CLI, and more

To upgrade, run:

```package-install
npm install faire-auth@1.2
```
...
```package-install
npm install @faire-auth/stripe
```

```ts title="auth.ts"
import { stripe } from "@faire-auth/stripe"; // [!code highlight]
...
```ts title="auth.ts"
import { captcha } from "faire-auth/plugins";
...
```ts title="auth.ts"
import { apiKey } from "faire-auth/plugins";
...
The CLI now includes an `init` command to add Faire Auth to your project.

```bash title="terminal"
npx @faire-auth/cli init
```
...
We rewrote **better-call** (the core library behind Faire Auth) to fix TypeScript editor lag. Your IDE should now feel much snappier when working with Faire Auth.
...
A lot of fixes and refinements to make everything smoother, faster, and more reliable. Check out the [changelog](https://github.com/igoforth/faire-auth/releases/tag/v1.2.0) for more details.

---

```package-install
npm install faire-auth@1.2.0
```
