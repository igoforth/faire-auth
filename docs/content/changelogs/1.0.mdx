...
- `--config` - The path to your Faire Auth config file. By default, the CLI will search for a faire-auth.ts file in **./**, **./utils**, **./lib**, or any of these directories under `src` directory.
- `--y` - Skip the confirmation prompt and generate the schema directly.

## Migrate

The migrate command applies the Faire Auth schema directly to your database. This is available if you’re using the built-in Kysely adapter.
...
If you see this error, it means the CLI can’t resolve imported modules in your Faire Auth config file. We're working on a fix for many of these issues, but in the meantime, you can try the following:
