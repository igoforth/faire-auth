import type { DBFieldAttribute, DBFieldType } from "@faire-auth/core/db";
import { createLogger } from "@faire-auth/core/env";
import type {
...
export async function getMigrations(config: FaireAuthOptions) {
	const betterAuthSchema = getSchema(config);
	const logger = createLogger(config.logger);

	// let { kysely: db, databaseType: dbType } = createKyselyAdapter(config);
	const adapter = createKyselyAdapter(config);
	if ("initialize" in adapter) await adapter.initialize();
	let { kysely: db, databaseType: dbType } = adapter;
