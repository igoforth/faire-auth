...
describe("adapter test", async (test) => {
	...
	const [authContext] = init(opts);
	const internalAdapter = authContext.internalAdapter;

	test("should create oauth user with custom generate id", async ({
		expect,
	}) => {
		const user = await runWithContext({ context: authContext }, () =>
	...
	test("should find session with custom userId", async ({ expect }) => {
	...
	test("should delete expired verification values on find", async ({
		expect,
	}) => {
		await runWithContext({ context: authContext }, () =>
		...
		const value = await runWithContext({ context: authContext }, () =>
			internalAdapter.findVerificationValue("test-id-1"),
		);
		expect(value).toMatchObject({
			identifier: "test-id-1",
		});
		const value2 = await runWithContext({ context: authContext }, () =>
			internalAdapter.findVerificationValue("test-id-1"),
		);
		...
		const value3 = await runWithContext({ context: authContext }, () =>
			internalAdapter.findVerificationValue("test-id-1"),
		);
		expect(value3).toMatchObject({
			identifier: "test-id-1",
		});
		const value4 = await runWithContext({ context: authContext }, () =>
			internalAdapter.findVerificationValue("test-id-1"),
		);
	...
	test("runs the after hook after adding user to db", async ({ expect }) => {
		const sampleUser = {
			name: "sample",
	...
	test("should calculate TTL correctly with Math.floor for secondary storage", async ({
		expect,
	}) => {
		...
		const [testCtx, resOpts] = init(testOpts);
		...
		await runWithContext({ context: testCtx }, () =>
		...
		await resOpts.secondaryStorage?.set(
		...
		await runWithContext({ context: testCtx }, () =>
			internalAdapter.updateUser(testUser.id, {
				name: "Updated Name",
			}),
		);
	...
	test("should create on secondary storage", async ({ expect }) => {
		// Create session
		const now = Date.now();
		...
		const session = await runWithContext({ context: authContext }, () =>
	...
	test("should delete on secondary storage", async ({ expect }) => {
	...
	test("should delete a single account", async ({ expect }) => {
		const user = await runWithContext({ context: authContext }, () =>
			internalAdapter.createUser({
		...
		const account = await runWithContext({ context: authContext }, () =>
		...
		let foundAccount = await runWithContext({ context: authContext }, () =>
			internalAdapter.findAccount(account.accountId),
		);
		...
		foundAccount = await runWithContext({ context: authContext }, () =>
			internalAdapter.findAccount(account.accountId),
		);
		expect(foundAccount).toBeNull();
	});

	test("should delete multiple accounts for a user", async ({ expect }) => {
		const user = await runWithContext({ context: authContext }, () =>
			internalAdapter.createUser({
		...
		let accounts = await runWithContext({ context: authContext }, () =>
		...
		accounts = await runWithContext({ context: authContext }, () =>
			internalAdapter.findAccounts(user.id),
		);
