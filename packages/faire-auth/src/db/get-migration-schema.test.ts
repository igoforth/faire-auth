...
describe.runIf(isPostgresAvailable)(
	...
	(test) => {
		...
		const customSchemaPool = new Pool({
			connectionString: `${getConnectionString(inject("postgresKysely"))}?options=-c search_path=${customSchema}`,
		});
		...
		test("should detect custom schema from search_path", async ({ expect }) => {
			// Use Pool with search_path option
			const config: FaireAuthOptions = {
		...
		test("should not be affected by tables in public schema when using custom schema", async ({
			expect,
		}) => {
		...
		test("should only inspect tables in public schema when using default connection", async ({
			expect,
		}) => {
			await publicPool.query(`DROP TABLE IF EXISTS public.user CASCADE`);
			await publicPool.query(`
		...
		test("should create tables in custom schema when running migrations", async ({
			expect,
		}) => {
...
// 			connectionString: `${getConnectionString(inject("postgresKysely"))}?options=-c search_path=${schema}`,
// 		});
...
// 		test("should use uuid for id when `advanced.database.generateId` is set to 'uuid'", async ({
// 			expect,
// 		}) => {
// 			const config: FaireAuthOptions = {
// 				database: schemaPool,
...
// 			const auth = faireAuth(config);

// 			const user = await auth.api.signUpEmail({
// 				json: {
// 					email: "genId@test.com",
// 					password: "test123456",
// 					name: "test user",
// 				},
// 			});
...
// 			expect(user.data?.user.id).toMatch(uuidRegex);
...
// 		test("should use GENERATED ALWAYS AS IDENTITY instead of SERIAL when `advanced.database.generateId` is set to 'serial'", async ({
// 			expect,
// 		}) => {
// 			const config: FaireAuthOptions = {
// 				database: schemaPool,
