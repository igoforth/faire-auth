...
// Uses the dedicated postgres-kysely2 container to avoid interference
// with the regular Kysely PG adapter test that runs in parallel.
let isPostgresAvailable = false;
try {
	const testPool = new Pool({
		connectionString: getConnectionString(inject("postgresKysely2")),
...
describe.runIf(isPostgresAvailable)(
	...
	(test) => {
		...
		const customSchemaPool = new Pool({
			connectionString: `${getConnectionString(inject("postgresKysely2"))}?options=-c search_path=${customSchema}`,
		});
