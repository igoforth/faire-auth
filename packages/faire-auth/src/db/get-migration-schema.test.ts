...
try {
	const testPool = new Pool({
		connectionString: "postgres://user:password@localhost:5434/faire_auth",
...
describe.runIf(isPostgresAvailable)(
	...
	() => {
		...
		const customSchemaPool = new Pool({
			connectionString: `postgres://user:password@localhost:5434/faire_auth?options=-c search_path=${customSchema}`,
		});
