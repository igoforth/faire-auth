...
interface AdapterTestOptions {
	getAdapter: (
		customOptions?: Omit<FaireAuthOptions, "database">,
	) => Promise<DBAdapter<FaireAuthOptions>> | DBAdapter<FaireAuthOptions>;
...
interface NumberIdAdapterTestOptions {
	getAdapter: (
		customOptions?: Omit<FaireAuthOptions, "database">,
	) => Promise<DBAdapter<FaireAuthOptions>>;
...
function adapterTest(
	...
	internalOptions?: {
		predefinedOptions: Omit<FaireAuthOptions, "database">;
	},
) {
	console.warn(
...
export function runNumberIdAdapterTest(opts: NumberIdAdapterTestOptions) {
	...
	describe("Should run number id specific tests", async (test) => {
		let idNumber = -1;
		...
		test.skipIf(opts.disableTests?.SHOULD_RETURN_A_NUMBER_ID_AS_A_RESULT)(
			...
			async ({ onTestFailed }) => {
				...
				expect(res.id).toBeTypeOf("string"); // we forcefully return all `id`s as strings. this is intentional.
	...
	describe("Should run normal adapter tests with number id enabled", async (test) => {
		beforeAll(async () => {
