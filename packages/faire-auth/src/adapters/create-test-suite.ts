import type { Account, Session, User, Verification } from "@faire-auth/core/db";
...
>(
	suiteName: string,
	config: {
		defaultFaireAuthOptions?: FaireAuthOptions;
		/**
		 * Helpful if the default faire auth options require migrations to be run.
		 */
	...
	tests: (
		helpers: {
			...
			getAuth: () => Promise<ReturnType<typeof faireAuth>>;
	...
	return (
		options?: {
			disableTests?:
				| Partial<Record<keyof Tests, boolean> & { ALL?: boolean }>
				| undefined;
		} & AdditionalOptions,
	) => {
		return async (helpers: {
			adapter: () => Promise<DBAdapter<FaireAuthOptions>>;
			log: Logger;
			...
			const wrapperAdapter = (overrideOptions?: FaireAuthOptions) => {
				const options = deepmerge(
					deepmerge(
						helpers.getFaireAuthOptions(),
						config?.defaultFaireAuthOptions || {},
					),
					overrideOptions || {},
				);
				const adapterConfig = {
			...
			const resetFaireAuthOptions = async () => {
				adapter = await helpers.adapter();
				await helpers.modifyFaireAuthOptions(
					config.defaultFaireAuthOptions || {},
				);
			...
			const modifyFaireAuthOptions = async (
				opts: FaireAuthOptions,
				shouldRunMigrations: boolean,
			) => {
				const res = helpers.modifyFaireAuthOptions(
					deepmerge(config?.defaultFaireAuthOptions || {}, opts),
				);
			...
			const fullTests = tests(
				{
					...
					getAuth: async () => {
						...
						const auth = faireAuth({
							...helpers.getFaireAuthOptions(),
							...(config?.defaultFaireAuthOptions || {}),
							database: (options: FaireAuthOptions) => {
					...
					insertRandom,
					modifyFaireAuthOptions,
					getFaireAuthOptions: helpers.getFaireAuthOptions,
					sortModels,
					tryCatch,
			...
			test(`\n${TTY_COLORS.fg.white}${" ".repeat(3)}${dash.repeat(35)} [${TTY_COLORS.fg.magenta}${suiteName}${TTY_COLORS.fg.white}] ${dash.repeat(35)}`, async () => {
				...
				if (config.defaultFaireAuthOptions && !allDisabled) {
					await helpers.modifyFaireAuthOptions(config.defaultFaireAuthOptions);
