...
export const normalTestSuite = createTestSuite("normal", {}, (helpers) => {
	...
	return {
		"init - tests": async () => {
			const opts = helpers.getFaireAuthOptions();
...
export const getNormalTestSuiteTests = ({
	...
	return {
		"create - should create a model": async () => {
			...
			const options = getFaireAuthOptions();
		...
		"findMany - should find many models with ends_with operator": async () => {
			...
			if (!ends_with)
				throw new TestError(
					"Expected first entry of users to have name",
					users,
				);
			const result = await adapter.findMany<User>({
				model: "user",
		...
		"findMany - should find many models with sortBy": async () => {
			...
			try {
				...
				console.log(expectedResult);
				console.log(`--------------------------------`);
				throw new TestError("Attempted to map result", result, error);
			}
			const options = getFaireAuthOptions();
			if (options.advanced?.database?.useNumberId) {
		...
		"findMany - should find many models with sortBy and limit and offset and where":
			async () => {
				...
				try {
					...
					console.log(`--------------------------------`);
					throw new TestError(
						"Expected result to be array with length 2 to equal exxpectedResult",
						result,
						error,
					);
				}
		...
		"deleteMany - should delete many models with numeric values": async () => {
			...
			if (!users[0] || !users[1] || !users[2]) {
				...
				throw new TestError("Users not found", users);
			}
