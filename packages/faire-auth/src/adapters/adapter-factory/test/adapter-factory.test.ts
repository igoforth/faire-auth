...
import type { CleanedWhere, Where } from "@faire-auth/core/db/adapter";
...
describe("Create Adapter Helper", async (test) => {
	...
	test("Should have the correct adapter id", ({ expect }) => {
		expect(adapter.id).toBe(adapterId);
	});

	test("Should use the id generator if passed into the faireAuth config", async ({
		expect,
	}) => {
		const adapter = await createTestAdapter({
			config: {
	...
	describe("Checking for the results of an adapter call, as well as the parameters passed into the adapter call", (test) => {
		describe("create", (test) => {
			...
			test('Should include an "id" in the result in all cases, unless "select" is used to exclude it', async () => {
				...
				expect(res2?.id).toBeTypeOf("undefined");
			...
			test('Should receive a generated id during the call, unless "disableIdGeneration" is set to true', async () => {
				...
				expect(createWithId.id).toBeTypeOf("string");
			...
			test("Should allow custom transform input", async ({ expect }) => {
			...
			test("Should allow custom transform output", async ({ expect }) => {
