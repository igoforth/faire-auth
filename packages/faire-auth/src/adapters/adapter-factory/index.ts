...
import { getColorDepth, logger, TTY_COLORS } from "@faire-auth/core/env";
...
const createAsIsTransaction =
	<O extends FaireAuthOptions>(adapter: DBAdapter<O>) =>
	<R>(fn: (trx: DBTransactionAdapter<O>) => R | Promise<R>) => {
		const res = fn(adapter);
		return isPromise(res) ? res : Promise.resolve(res);
	};

export type AdapterFactory = <O extends FaireAuthOptions>(
	options: O,
) => DBAdapter<O>;
...
export const createAdapterFactory =
	...
	<O extends FaireAuthOptions>(options: O): DBAdapter<O> => {
		...
		// End-user's Faire-Auth instance's schema
		...
		const getDefaultFieldName = ({
			...
			// Plugin `schema`s can't define their own `id`. Faire-auth auto provides `id` to every schema model.
			...
			if (!f) {
				...
				throw new FaireAuthError(`Field ${field} not found in model ${model}`);
			}
			return field;
		};
		...
		const getDefaultModelName = (model: string) => {
			...
			if (!m) {
				...
				debugLog(`Schema:`, schema);
				throw new FaireAuthError(`Model "${model}" not found in schema`);
			}
			return m;
		};
					...
					? {
							defaultValue() {
								...
								if (options.advanced?.generateId !== undefined) {
									logger.warn(
										"Your Faire Auth config includes advanced.generateId which is deprecated. Please use advanced.database.generateId instead. This will be removed in future releases.",
									);
			...
			return where.map((w) => {
				...
				if (operator === "in") {
					if (!Array.isArray(value)) {
						throw new FaireAuthError("Value must be an array");
					}
				}
		...
		let lazyLoadTransaction: DBAdapter<O>["transaction"] | null = null;
		const adapter: DBAdapter<O> = {
			transaction: (cb) => {
