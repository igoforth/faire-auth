...
export const createKyselyAdapter = (config: FaireAuthOptions) => {
	...
	const toReturn = {
		...
		transaction: undefined,
		async initialize() {
			if (toReturn.kysely == null) {
				if ("fileControl" in db) {
					console.log("found file control");
					const { BunSqliteDialect } = await import("./bun-sqlite-dialect");
					dialect = new BunSqliteDialect({ database: db });
				}
				if ("createSession" in db && typeof window === "undefined") {
					console.log("found create session");
					let DatabaseSync:
				...
				if (dialect) toReturn.kysely = new Kysely<any>({ dialect });
			}
		},
	};
