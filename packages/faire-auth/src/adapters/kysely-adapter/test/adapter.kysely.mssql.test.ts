...
const {
	password,
	username: userName,
	database = "master", // Start with master database, will create faire_auth if needed
	port,
	host: server,
} = inject("mssql");
...
// Create faire_auth database if it doesn't exist
const ensureDatabaseExists = async () => {
	try {
		console.log("Ensuring faire_auth database exists...");
		await kyselyDB.getExecutor().executeQuery({
			sql: `
				IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = 'faire_auth')
				BEGIN
					CREATE DATABASE faire_auth;
					PRINT 'Database faire_auth created successfully';
				END
				ELSE
				BEGIN
					PRINT 'Database faire_auth already exists';
				END
			`,
			parameters: [],
...
const query = async (sql: string, timeoutMs: number = 30000) => {
	...
	try {
		...
		// Ensure we're using the faire_auth database for queries
