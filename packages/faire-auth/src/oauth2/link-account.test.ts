...
describe("oauth2 - email verification on link", async (test) => {
	...
	async function linkGoogleAccount() {
		...
		const captureCookies = createCookieCapture(oAuthHeaders);
		...
		expect(signInRes.data?.success).toBe(true as true);
		expect(signInRes.data?.data.redirect).toBe(true as true);

		const state = new URL(signInRes.data!.data.url!).searchParams.get("state");
		expect(state).not.toBeNull();

		await client.$fetch("/callback/google", {
			query: { state: state!, code: "test_code" },
			method: "GET",
	...
	test("should update emailVerified when linking account with verified email", async ({
		expect,
	}) => {
	...
	test("should not update emailVerified when provider reports unverified", async ({
		expect,
	}) => {
	...
	test("should not update emailVerified when email addresses don't match", async ({
		expect,
	}) => {
	...
	test("should handle already verified emails gracefully", async ({
		expect,
	}) => {
