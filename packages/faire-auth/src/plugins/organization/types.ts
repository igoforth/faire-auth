...
import type { FieldAttribute } from "@faire-auth/core/types";
...
export interface OrganizationOptions {
	...
	invitationLimit?:
		| number
		| ((
				data: { user: User; organization: Organization; member: Member },
				ctx: AuthContext,
		  ) => Promise<number> | number);
	/**
	...
	schema?: {
		session?: {
			fields?: { activeOrganizationId?: string; activeTeamId?: string };
		};
		organization?: {
			modelName?: string;
