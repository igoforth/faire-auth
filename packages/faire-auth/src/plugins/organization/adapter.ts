...
export const getOrgAdapter = <O extends OrganizationOptions>(
	...
	return {
		...
		createOrganization: async (data: {
			organization: OrganizationInput & // This represents the additional fields from the plugin options
				Record<string, any>;
		}) => {
			...
			>({
				...
				data: {
					...
					...(data.organization.metadata && {
						metadata: JSON.stringify(data.organization.metadata),
					}),
				},
			});
			...
			const members = await Promise.all([
				adapter.findMany<Member>({
					...
					where: [
						{ field: "organizationId", value: data.organizationId },
						...(data.filter?.field
							? [{ field: data.filter.field!, value: data.filter.value }]
							: []),
					],
					limit: data.limit ?? options?.membershipLimit ?? 100,
					offset: data.offset ?? 0,
					...(data.sortBy && {
						sortBy: { field: data.sortBy, direction: data.sortOrder ?? "asc" },
					}),
				}),
				adapter.count({
					model: "member",
		...
		createMember: async (
			data: Omit<MemberInput, "id"> & // Additional fields from the plugin options
				Record<string, any>,
		) => {
			const member = await adapter.create<
				typeof data,
				Member & InferAdditionalFieldsFromPluginOptions<"member", O, false>
			>({ model: "member", data: { ...data, createdAt: new Date() } });
			return member;
		},
		...
		findFullOrganization: async ({
			...
			return { ...org, invitations, members: membersWithUsers, teams };
		},
		...
		findTeamById: async <IncludeMembers extends boolean = false>({
			teamId,
			organizationId,
			includeTeamMembers,
		}: {
		...
		listTeamsByUser: async (data: { userId: string }) => {
			...
			const teams = await adapter.findMany<Team>({
				model: "team",
				where: [
					{ field: "id", operator: "in", value: members.map((m) => m.teamId) },
				],
			});
