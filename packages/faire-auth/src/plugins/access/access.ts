...
export const role = <TStatements extends Statements>(
	...
	authorize: <K extends keyof TStatements>(
		request: {
			[key in K]?:
				| TStatements[key]
				| { actions: TStatements[key]; connector: "OR" | "AND" };
		},
		...
		for (const [requestedResource, requestedActions] of Object.entries(
			...
			if (Array.isArray(requestedActions))
				...
				} else throw new FaireAuthError("Invalid access control request");
			}
...
export const createAccessControl = <const TStatements extends Statements>(
	s: TStatements,
) => ({
	newRole: <K extends keyof TStatements>(statements: Subset<K, TStatements>) =>
		role<Subset<K, TStatements>>(statements),
	statements: s,
});
