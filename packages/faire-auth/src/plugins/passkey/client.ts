...
export const getPasskeyActions = (
	$fetch: BetterFetch,
	{ $listPasskeys }: { $listPasskeys: ReturnType<typeof atom<any>> },
) => {
	const signInPasskey = async (
		...
		const response = await $fetch<PublicKeyCredentialRequestOptionsJSON>(
			...
			{ method: "POST", body: { email: opts?.email } },
		);
		if (!response.data) {
			return response;
		}
		...
		try {
			...
			const verified = await $fetch<{ passkey: Passkey }>(
				"/passkey/verify-registration",
				{
					...opts?.fetchOptions,
					...fetchOpts,
					body: { response: res, name: opts?.name },
					method: "POST",
				},
			);
			if (!verified.data) {
			...
			if (e instanceof WebAuthnError) {
				...
				return {
					data: null,
					error: { message: e.message, status: 400, statusText: "BAD_REQUEST" },
				};
			}
			return {
				data: null,
...
export const passkeyClient = () => {
	...
	return {
		...
		getAtoms($fetch: BetterFetch) {
			...
			return { listPasskeys, $listPasskeys };
		},
		pathMethods: {
