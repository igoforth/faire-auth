...
beforeEach(() => {
	vi.resetModules();
	vi.clearAllMocks();
	process.env.FAIRE_AUTH_TELEMETRY = "";
	process.env.FAIRE_AUTH_TELEMETRY_DEBUG = "";
});

describe("telemetry", (test) => {
	test("publishes events when enabled", async ({ expect }) => {
		let event: TelemetryEvent | undefined;
		const track = vi.fn().mockImplementation(async (e) => {
			event = e;
		});
		const { initPromise } = createTelemetry(
			{
		...
		expect(event).toMatchObject({
			...
			payload: {
				config: {
					...
					onAPIError: { errorURL: undefined, onError: false, throw: undefined },
	...
	test("does not publish when disabled via env", async ({ expect }) => {
		process.env.FAIRE_AUTH_TELEMETRY = "false";
		let event: TelemetryEvent | undefined;
		const track = vi.fn().mockImplementation(async (e) => {
			event = e;
		});
	...
	test("does not publish when disabled via option", async ({ expect }) => {
		let event: TelemetryEvent | undefined;
		const track = vi.fn().mockImplementation(async (e) => {
			event = e;
		});
		const { initPromise } = createTelemetry(
			{ baseURL: "http://localhost", telemetry: { enabled: false } },
			{ customTrack: track, skipTestCheck: true },
		);
		await initPromise;
		expect(event).toBeUndefined();
	...
	test("shouldn't fail cause track isn't being reached", async ({ expect }) => {
		const { initPromise } = createTelemetry(
			{ baseURL: "http://localhost" },
			{
				customTrack() {
		...
		await expect(initPromise).resolves.not.throw(Error);
	});

	test("initializes without Node built-ins in edge-like env (no process.cwd)", async ({
		expect,
	}) => {
