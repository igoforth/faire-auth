import type { ProviderOptions } from "./oauth-provider";
import { generateCodeChallenge } from "./utils";

export const createAuthorizationURL = async ({
	options,
	authorizationEndpoint,
	state,
	codeVerifier,
	scopes,
	claims,
	redirectURI,
	duration,
	prompt,
	accessType,
	responseType,
	display,
	loginHint,
	hd,
	responseMode,
	additionalParams,
	scopeJoiner,
}: {
	id: string;
	options: ProviderOptions;
	redirectURI: string;
	authorizationEndpoint: string;
	state: string;
	// TODO: maybe remove undefined union in future to assert no properties
	// will ever be undefined
	codeVerifier?: string | undefined;
	scopes: string[];
	claims?: string[] | undefined;
	duration?: string | undefined;
	prompt?: string | undefined;
	accessType?: string | undefined;
	responseType?: string | undefined;
	display?: string | undefined;
	loginHint?: string | undefined;
	hd?: string | undefined;
	responseMode?: string | undefined;
	additionalParams?: Record<string, string> | undefined;
	scopeJoiner?: string | undefined;
}) => {
	const url = new URL(authorizationEndpoint);
	url.searchParams.set("response_type", responseType ?? "code");
	const primaryClientId = Array.isArray(options.clientId)
		? options.clientId[0]
		: options.clientId;
	url.searchParams.set("client_id", primaryClientId);
	url.searchParams.set("state", state);
	url.searchParams.set("scope", scopes.join(scopeJoiner ?? " "));
	url.searchParams.set("redirect_uri", options.redirectURI ?? redirectURI);
	if (duration != null) url.searchParams.set("duration", duration);
	if (display != null) url.searchParams.set("display", display);
	if (loginHint != null) url.searchParams.set("login_hint", loginHint);
	if (prompt != null) url.searchParams.set("prompt", prompt);
	if (hd != null) url.searchParams.set("hd", hd);
	if (accessType != null) url.searchParams.set("access_type", accessType);
	if (responseMode != null) url.searchParams.set("response_mode", responseMode);
	if (codeVerifier != null) {
		const codeChallenge = await generateCodeChallenge(codeVerifier);
		url.searchParams.set("code_challenge_method", "S256");
		url.searchParams.set("code_challenge", codeChallenge);
	}
	if (claims) {
		const claimsObj = claims.reduce<Record<string, null>>((acc, claim) => {
			acc[claim] = null;
			return acc;
		}, {});
		url.searchParams.set(
			"claims",
			JSON.stringify({
				id_token: { email: null, email_verified: null, ...claimsObj },
			}),
		);
	}
	if (additionalParams) {
		Object.entries(additionalParams).forEach(([key, value]) => {
			url.searchParams.set(key, value);
		});
	}
	return url;
};
