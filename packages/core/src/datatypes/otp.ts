import type { SHAFamily } from "./types";
...
async function generateHOTP(
	secret: string,
	{
		...
		digits,
		hash = "SHA-1",
	}: { counter: number; digits?: number; hash?: SHAFamily | undefined },
) {
...
async function generateTOTP(
	secret: string,
	options?: { period?: number; digits?: number; hash?: SHAFamily },
) {
...
async function verifyTOTP(
	...
	}: { period?: number; window?: number; digits?: number; secret: string },
) {
	const milliseconds = period * 1000;
	const counter = Math.floor(Date.now() / milliseconds);
...
export const createOTP = (
	secret: string,
	opts?: { digits?: number; period?: number | undefined },
) => {
