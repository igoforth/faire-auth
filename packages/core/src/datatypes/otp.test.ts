...
describe("HOTP and TOTP Generation Tests", (test) => {
	test("should generate a valid HOTP for a given counter", async ({
		...
		const counter = 1;
		const digits = 6;
		const otp = await createOTP(key, { digits }).hotp(counter);
	...
	test("should throw error if digits is not between 1 and 8", async ({
		...
		await expect(createOTP(key, { digits: 9 }).hotp(counter)).rejects.toThrow(
			"Digits must be between 1 and 8",
		);
		await expect(createOTP(key, { digits: 0 }).hotp(counter)).rejects.toThrow(
			"Digits must be between 1 and 8",
		);
	});
	...
	test("should generate a valid TOTP based on current time", async ({
		...
		const otp = await createOTP(secret, { digits }).totp();
		expect(otp).toBeTypeOf("string");
		expect(otp.length).toBe(digits);
	});
	...
	test("should generate different OTPs after each time window", async ({
		...
		const otp1 = await createOTP(secret, { period: seconds, digits }).totp();
		vi.useFakeTimers();
		await vi.advanceTimersByTimeAsync(30000);
		const otp2 = await createOTP(secret, { period: seconds, digits }).totp();
